...
from core.utils.lmdb_util import LmdbMaker
...
def convert_celeba_tfrecords(tf_file, log_resolution, save_root, save_type='img', compress_level=1):
    ...
    if save_type == 'lmdb':
        save_path = os.path.join(
            save_root, f'celeba_{2**log_resolution}_{phase}.lmdb')
        lmdb_maker = LmdbMaker(save_path)
    elif save_type == 'img':
        save_path = os.path.join(
            save_root, f'celeba_{2**log_resolution}_{phase}')
    else:
...
def make_ffhq_lmdb_from_imgs(folder_path, log_resolution, save_root, save_type='lmdb', compress_level=1):
    ...
    if save_type == 'lmdb':
        save_path = os.path.join(
            save_root, f'ffhq_{2**log_resolution}_crop1.2.lmdb')
        lmdb_maker = LmdbMaker(save_path)
    else:
        raise ValueError('Wrong save type.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_resolution', type=int,
                        default=10, help='Log scale of resolution.')
    parser.add_argument('--save_root', type=str,
                        default='datasets/ffhq/', help='Save root path.')
    parser.add_argument(
